...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($pending2faUserId && isset($_POST['otp'])) {
        ...
        if (empty($email) || empty($password)) {
        ...
        } elseif (mb_strlen($password) > 1000) {
            ...
            } elseif ($user->isBanned($userData)) {
                // Пользователь заблокирован, но позволяем вход для показа предупреждений
                $banInfo = $user->getBanInfo($userData['id']);
                $_SESSION['user_banned'] = true;
                $_SESSION['ban_info'] = $banInfo;
                // Продолжаем проверку пароля для входа
...
.auth-card { width: 100%; max-width: 440px; border-radius: 16px; border: none; box-shadow: 0 4px 24px rgba(0,0,0,.08); }
...
.input-group .btn-outline-secondary { border-color: #dee2e6; border-radius: 0 10px 10px 0; color: #6c757d; }
...
.divider { display: flex; align-items: center; gap: .75rem; color: #adb5bd; font-size: .85rem; }
            ...
            <div class="card-body p-4">
                ...
                <form action="login.php" method="POST" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <span class="input-group-text border-end-0 bg-white"><i class="bi bi-lock text-secondary"></i></span>
                            <input type="password" id="password" name="password"
                                class="form-control border-start-0 border-end-0 ps-0"
                                placeholder="Ваш пароль" autocomplete="current-password" required>
                            <button type="button" class="btn btn-outline-secondary border-start-0" onclick="togglePass('password',this)">
                                <i class="bi bi-eye"></i>
                            </button>
                        </div>
                    </div>
